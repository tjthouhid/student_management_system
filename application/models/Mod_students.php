    ...
    /**
      ...
      function save_student_info($postedData,$image_name){
        $this->load->helper('encrypt');
          ...
          $postedData['student_password']=encrypt_pass($postedData['student_password']);
          if($image_name!==""){
            $postedData['student_photo']=$image_name;
          }
        $query= $this->db->insert('students',$postedData);
         if($query){
       ...
       /**
         ...
         * @version 2017-11-26
         */
      function save_student_course($cs_data){
          
          
        $query= $this->db->insert('students_courses',$cs_data);
         if($query){
          return $this->db->insert_id();  
         }else{
          return false;
     ...
     function get_student_datas($user_id){
      
      $this->db->select("st.*,sc.course_id,cs.course_name,cs.course_price");
      ...
      $this->db->where('st.student_id',$user_id);
      $query=$this->db->get();
      if($query->num_rows()>0){
     ...
     function get_student_payment_datas($user_id){
      
      $this->db->select("sp.*");
      $this->db->from("students_payments sp");
    
      $this->db->where('sp.student_id',$user_id);
      $query=$this->db->get();
      if($query->num_rows()>0){
        return $result=$query->result();
      }else{
     ...
     function checkEmail($student_email){
      $result=$this->db->get_where('students', array('student_email' => $student_email ));
